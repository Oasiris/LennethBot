...
// === Logger Setup ===
const logger = logger_1.instantiateLogger();
 ...
 */
const COMMANDS = commands_1.default;
/**
 * Class full of functions that do neat things for incoming messages.
 */
class MessageUtil {
    static isCommand(string, commandBank = commands_1.default) {
        const isNull = val => val === null;
        return ramda_1.not(isNull(string.match(COMMAND_PREFIX_REGEX)));
    }
    static purifyCommand(fullCommandString, commandBank = commands_1.default) {
...
function handleMessageEvent(msg) {
    ...
    if (intent.hasIntent) {
        MessageUtil.executeActionFromCommand(intent, botActions, onValidCommand, onInvalidCommand);
    }
}
...
function createChannelMap() {
    ...
    channelNameMap = {};
    let textChannels = channelMapById.array().filter(ramda_1.propEq('type', 'text'));
...
// === CLI Functionality ===
bot.on('ready', () => cli_1.startCli(bot, logger));
